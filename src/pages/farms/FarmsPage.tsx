...
export function FarmsPage() {
  ...
  async function loadFarmInfoList(isUpload?: boolean, isSignedIn?: boolean) {
    useEffect(() => {
      const sort_from_url = new URLSearchParams(location.search).get('sort');
      const status_from_url = +new URLSearchParams(location.search).get(
        'status'
      );
      if (
              ...
              {Object.entries(rewardList).length > 0 ? (
                <>
                  <div
                    ...
                    ref={rewardRef}
                  >
                    {Object.entries(rewardList).map(
                      (rewardToken: any, index) => (
                        <WithdrawView
                          key={index}
                          index={index}
                          data={rewardToken}
  ...
  function FarmView({
    ...
    function isPending(data: FarmInfo) {
      ...
      if (farmsData.length > 1) {
        for (let i = 0; i < farmsData.length; i++) {
          if (
            moment.unix(farmsData[i].start_at).valueOf() > moment().valueOf()
          ) {
            pending = true;
          } else {
  ...
  function ActionModal(
    ...
    const [displayTokenData, setDisplayTokenData] = useState<
      Record<string, any>
    >({});
