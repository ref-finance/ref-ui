...
const AddLiquidityProviderData = createContext(null);
...
function AddYourLiquidityPageV3Reverse() {
  ...
  useEffect(() => {
    // init
    if (currentSelectedPool?.pool_id) {
      console.log('come in currentSelectedPool', currentSelectedPool);
      const { current_point, point_delta } = currentSelectedPool;
      const n = get_slot_number_in_a_bin();
      const bin_width = n * point_delta;
      SET_SLOT_NUMBER(n);
...
function PointsComponent() {
  const { pair_is_reverse } = useContext(AddLiquidityProviderData);
  if (pair_is_reverse) {
    return <SetPointsComponentReverse></SetPointsComponentReverse>;
  } else {
    return <SetPointsComponent></SetPointsComponent>;
  }
}
...
function SetPointsComponentReverse() {
  ...
  useEffect(() => {
    if (!isInvalid(leftPoint) && !isInvalid(rightPoint)) {
      ...
      // todo
      console.log(
        'real_leftPoint, real_rightPoint, real_current_point',
        rightPoint,
        leftPoint,
        currentPoint
      );
