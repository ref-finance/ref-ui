...
export default function AddYourLiquidityPageV3() {
  // return <AddYourLiquidityPageV3Forward></AddYourLiquidityPageV3Forward>
  return <AddYourLiquidityPageV3Reverse></AddYourLiquidityPageV3Reverse>
}
function AddYourLiquidityPageV3Forward() {
      ...
      >
        ...
        >
          <div className="flex items-start justify-between xs:flex-col md:flex-col">
            ...
            {/* left area */}
            {currentSelectedPool && currentSelectedPool.pool_id ? (
              <SetPointsComponentReverse></SetPointsComponentReverse>
            ) : null}
            {/* right area */}
            <div
              style={{ width: mobileDevice ? '' : '400px' }}
...
function SetPointsComponentReverse() {
  ...
  useEffect(() => {
    if (currentSelectedPool?.pool_id && !switch_pool_loading) {
      const { current_point } = currentSelectedPool;
      const left_point = get_bin_point_by_point(
        current_point + BIN_WIDTH * RADIUS_DEFAULT_NUMBER
      );
      const right_point = left_point - BIN_WIDTH * RADIUS_DEFAULT_NUMBER * 2;
      setTargetPoint(current_point);
      setRadius(RADIUS_DEFAULT_NUMBER);
      setLeftPoint(left_point);
      setRightPoint(right_point);
  ...
  useEffect(() => {
    if (!isInvalid(leftPoint) && !isInvalid(rightPoint)) {
      // effect bin
      const diff = leftPoint - rightPoint;
      const bin_number_temp = diff / BIN_WIDTH;
      setBinNumber(bin_number_temp);
      // effect right area
      pointChange({ leftPoint, rightPoint, currentPoint });
  ...
  // 修改radius-->合适的左右点位 --->合适的radius
  function changeRadius(radius: number) {
    let appropriate_left_point = get_bin_point_by_point(
      targetPoint + BIN_WIDTH * radius
    );
    let appropriate_right_point = get_bin_point_by_point(
  ...
  // 修改 targetPrice-->合适的左右点位--->合适的targetPrice
  function handleTargetPriceToAppropriatePoint(price: string) {
    let appropriate_target_point = get_bin_point_by_price(reverse_price(price));
    const appropriate_left_point = get_bin_point_by_point(
      appropriate_target_point + BIN_WIDTH * radius
    );
    const appropriate_right_point = get_bin_point_by_point(
      appropriate_left_point - BIN_WIDTH * radius * 2
    );
  ...
  function getLeftPrice() {
    if (currentSelectedPool && currentSelectedPool.pool_id && !isInvalid(leftPoint)) {
      const price = reverse_price(get_price_by_point(leftPoint));
      if (new BigNumber(price).isLessThan('0.00000001')) {
        return price;
      } else {
        return toPrecision(price.toString(), 8);
  ...
  function getRightPrice() {
    if (currentSelectedPool && currentSelectedPool.pool_id && !isInvalid(rightPoint)) {
      const price = reverse_price(get_price_by_point(rightPoint))
      if (new BigNumber(price).isLessThan('0.00000001')) {
        return price;
      } else {
        return toPrecision(price.toString(), 8);
  ...
  function getTargetPrice() {
    if (currentSelectedPool && currentSelectedPool.pool_id && !isInvalid(targetPoint)) {
      let price = reverse_price(get_price_by_point(targetPoint));
      if (new BigNumber(price).isLessThan('0.00000001')) {
        return price;
      } else {
        return toPrecision(price.toString(), 8);
  ...
  function get_point_by_price(price: string) {
    const { point_delta } = currentSelectedPool;
    const decimalRate_point = Math.pow(10, token_y_decimals) / Math.pow(10, token_x_decimals);
  ...
  function get_price_by_point(point: number) {
    const decimalRate_price =
      Math.pow(10, token_x_decimals) /
      Math.pow(10, token_y_decimals);
