...
//       return liquidity
function compute_liquidity(left_point:number, right_point:number, amount_x:string, amount_y:string, current_point:number) {
  const { x, y } = compute_deposit_xy_per_unit(left_point, right_point, current_point);
  let xl = '0';
  let yl = '0';
  if (+x > 0) {
...
//       return (x, y)
function compute_deposit_xy_per_unit(left_point: number, right_point: number, current_point: number) {
  ...
  if (right_point > current_point) {
    ...
    x = get_amount_x_unit_liquidity(xr_left, right_point, right_price, current_price);
  }
  return {x, y}
}
...
function get_amount_y_unit_liquidity(left_price:string, price_right:string, current_price:string) {
...
//    return mul_fraction_ceil(pow_96(), numerator, denominator)
function get_amount_x_unit_liquidity(left_point:number, right_point:number, price_right:string, current_price:string) {
  const sqrt_price_pr_pc =  get_sqrt_price(right_point - left_point + 1);
  const sqrt_price_pr_pd = get_sqrt_price(right_point + 1);
  const numerator = new BigNumber(sqrt_price_pr_pc).minus(current_price).toFixed();
  const denominator = new BigNumber(sqrt_price_pr_pd).minus(price_right).toFixed();
  return mul_fraction_ceil(pow_96(), numerator, denominator);
}
// def pow_96():
//    return 1<<96
...
//    else:
//       return (res+1)
function mul_fraction_ceil(number:string, _numerator:string, _denominator:string) {
  const res = new BigNumber(number).multipliedBy(_numerator).dividedToIntegerBy(_denominator);
  if (new BigNumber(number).multipliedBy(_numerator).modulo(_denominator).toFixed() == '0') {
    return res.toFixed();
  } else {
