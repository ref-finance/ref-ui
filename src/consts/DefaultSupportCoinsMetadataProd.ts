const DefaultSupportedCoinsMetadataProd = {
  "wrap.near": {
    id: "wrap.near",
    version: "0.1.0",
    name: "Wrapped NEAR fungible token",
    symbol: "wNEAR",
    icon: "https://fluxprotocol.eth.link/static/media/wrapped-near.8b3a5e4b.svg",
    reference: "https://github.com/near/core-contracts/tree/master/w-near-141",
    decimals: 24,
  },
  "6b175474e89094c44da98b954eedeac495271d0f.factory.bridge.near": {
    id: "6b175474e89094c44da98b954eedeac495271d0f.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nDAI",
    symbol: "nDAI",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "berryclub.ek.near": {
    id: "berryclub.ek.near",
    version: "0.1.0",
    name: "Banana",
    symbol: "BANANA",
    icon: "🍌",
    url: "https://github.com/evgenykuzyakov/berryclub",
    decimals: 18,
  },
  "dac17f958d2ee523a2206206994597c13d831ec7.factory.bridge.near": {
    id: "dac17f958d2ee523a2206206994597c13d831ec7.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nUSDT",
    symbol: "nUSDT",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 6,
  },
  "1f9840a85d5af5bf1d1762f925bdaddc4201f984.factory.bridge.near": {
    id: "1f9840a85d5af5bf1d1762f925bdaddc4201f984.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nUNI",
    symbol: "nUNI",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "514910771af9ca656af840dff83e8264ecf986ca.factory.bridge.near": {
    id: "514910771af9ca656af840dff83e8264ecf986ca.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nLINK",
    symbol: "nLINK",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48.factory.bridge.near": {
    id: "a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nUSDC",
    symbol: "nUSDC",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 6,
  },
  "2260fac5e5542a773aa44fbcfedf7c193bc2c599.factory.bridge.near": {
    id: "2260fac5e5542a773aa44fbcfedf7c193bc2c599.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nWBTC",
    symbol: "nWBTC",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 8,
  },
  "7fc66500c84a76ad7e9c93437bfc5ac33e2ddae9.factory.bridge.near": {
    id: "7fc66500c84a76ad7e9c93437bfc5ac33e2ddae9.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nAAVE",
    symbol: "nAAVE",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "a0b73e1ff0b80914ab6fe0444e65848c4c34450b.factory.bridge.near": {
    id: "a0b73e1ff0b80914ab6fe0444e65848c4c34450b.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nCRO",
    symbol: "nCRO",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 8,
  },
  "50d1c9771902476076ecfc8b2a83ad6b9355a4c9.factory.bridge.near": {
    id: "50d1c9771902476076ecfc8b2a83ad6b9355a4c9.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nFTT",
    symbol: "nFTT",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "4fabb145d64652a948d72533023f6e7a623c7c53.factory.bridge.near": {
    id: "4fabb145d64652a948d72533023f6e7a623c7c53.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nBUSD",
    symbol: "nBUSD",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "6f259637dcd74c767781e37bc6133cd6a68aa161.factory.bridge.near": {
    id: "6f259637dcd74c767781e37bc6133cd6a68aa161.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nHT",
    symbol: "nHT",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "6b3595068778dd592e39a122f4f5a5cf09c90fe2.factory.bridge.near": {
    id: "6b3595068778dd592e39a122f4f5a5cf09c90fe2.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nSUSHI",
    symbol: "nSUSHI",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "c011a73ee8576fb46f5e1c5751ca3b9fe0af2a6f.factory.bridge.near": {
    id: "c011a73ee8576fb46f5e1c5751ca3b9fe0af2a6f.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nSNX",
    symbol: "nSNX",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "c944e90c64b2c07662a292be6244bdf05cda44a7.factory.bridge.near": {
    id: "c944e90c64b2c07662a292be6244bdf05cda44a7.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nGRT",
    symbol: "nGRT",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "9f8f72aa9304c8b593d555f12ef6589cc3a579a2.factory.bridge.near": {
    id: "9f8f72aa9304c8b593d555f12ef6589cc3a579a2.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nMKR",
    symbol: "nMKR",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "c00e94cb662c3520282e6f5717214004a7f26888.factory.bridge.near": {
    id: "c00e94cb662c3520282e6f5717214004a7f26888.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nCOMP",
    symbol: "nCOMP",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "0bc529c00c6401aef6d220be8c6ea1667f6ad93e.factory.bridge.near": {
    id: "0bc529c00c6401aef6d220be8c6ea1667f6ad93e.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nYFI",
    symbol: "nYFI",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "c02aaa39b223fe8d0a0e5c4f27ead9083c756cc2.factory.bridge.near": {
    id: "c02aaa39b223fe8d0a0e5c4f27ead9083c756cc2.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nWETH",
    symbol: "nWETH",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "0316eb71485b0ab14103307bf65a021042c6d380.factory.bridge.near": {
    id: "0316eb71485b0ab14103307bf65a021042c6d380.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "nHBTC",
    symbol: "nHBTC",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
  "111111111117dc0aa78b770fa6a738034120c302.factory.bridge.near": {
    id: "111111111117dc0aa78b770fa6a738034120c302.factory.bridge.near",
    spec: "ft-1.0.0",
    name: "n1INCH",
    symbol: "n1INCH",
    icon:
      "https://near.org/wp-content/themes/near-19/assets/img/brand-icon.png",
    reference: "",
    reference_hash: "",
    decimals: 18,
  },
};

export default DefaultSupportedCoinsMetadataProd;
