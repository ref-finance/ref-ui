...
export const RemovePoolV3 = (props: any) => {
  ...
  return <RemovePoolV3Reverse props={props}></RemovePoolV3Reverse>
  // return <RemovePoolV3Forward props={props}></RemovePoolV3Forward>
 ...
 * @returns 
 */
export const RemovePoolV3Forward = ({ props }: any) => {
  const {
    tokenMetadata_x_y,
    poolDetail,
    tokenPriceList,
...
/**
 * 
 * @param props 逆向的操作思路是：展示层价格 是反的，数据层，点位是不变的。
 * @returns 
 */
export const RemovePoolV3Reverse = ({ props }: any) => {
  const {
    tokenMetadata_x_y,
    poolDetail,
    tokenPriceList,
  ...
  useEffect(() => { // =======》ok
    if (minBoxPoint && maxBoxPoint) {
      const bin_amount = get_bin_amount_by_points(maxBoxPoint, minBoxPoint);
      setBinBoxAmount(bin_amount);
    }
  }, [minBoxPoint, maxBoxPoint]);
  useEffect(() => {
  ...
  function get_user_points_range() {
    ...
    const min_price = reverse_price(get_bin_price_by_point(max_point));
    const max_price = reverse_price(get_bin_price_by_point(min_point));
    const max_bin_amount = get_bin_amount_by_points(min_point, max_point);
    setMinPoint(min_point);
    setMaxPoint(max_point);
    setMinPrice(min_price);
  ...
  function handleMinBoxPriceToAppropriatePoint() {
    ...
    if (big_price.lt(minPrice)) {
      ...
      appropriate_price = reverse_price(get_bin_price_by_point(appropriate_point));
    }
    setMinBoxPrice(appropriate_price);
    setMinBoxPoint(appropriate_point);
  }
  ...
  function handleBinAmountToAppropriateAmount(binAmount: number) {
    ...
    if (removeType == 'left') {
      const right_box_point = maxPoint - binWidth * appropriate_amount;
      const right_box_price = reverse_price(get_bin_price_by_point(right_box_point));
      setMaxBoxPoint(right_box_point);
      setMaxBoxPrice(right_box_price);
    } else if (removeType == 'right') {
      const left_box_point = minPoint + binWidth * appropriate_amount;
      const left_box_price = reverse_price(get_bin_price_by_point(left_box_point));
      setMinBoxPoint(left_box_point);
      setMinBoxPrice(left_box_price);
    } 
    setBinBoxAmount(appropriate_amount.toString());
  ...
  function get_un_deleted_range(liquidity: UserLiquidityInfo) {
    const { left_point, right_point } = liquidity;
    // intersection part
    const intersection_l = Math.min(right_point, minBoxPoint);
    const intersection_r = Math.max(left_point, maxBoxPoint);
    // intersection part
    let un_intersection_l;
    let un_intersection_r;
    if (removeType == 'left') {
