...
export default function DclChart(props:IDclChartProps) {
  if (props.reverse) {
    return <DclChartReverse {...props}></DclChartReverse>
  } else {
    return <DclChartForward {...props}></DclChartForward>
  }
}
function DclChartForward({
  pool_id,
  leftPoint,
  rightPoint,
  setLeftPoint,
...
function DclChartReverse({
  ...
  useEffect(() => { // =========> ok
    if (
      ...
      const scale = scaleAxis();
      const newPoint = dragLeftPoint;
      const newPrice = reverse_price(get_price_by_point(newPoint));
      const movePercent = diffPrices(newPrice);
      const x = scale(+newPrice) - dragBarWidth / 2;
      d3.select(`${randomId} .drag-left`).attr(
        'transform',
  ...
  function getChartDataListInRange() {
    ...
    const price_range_point_max = get_point_by_price(reverse_price(price_range_min));
    const price_range_point_min = get_point_by_price(reverse_price(price_range_max));
    const bin_width = getConfig().bin * pool.point_delta;
    const chartDataListInRange = chartDataList.filter((d: IChartData) => {
      const { point } = d;
      const point_right = point + bin_width;
  ...
  function get_price_range_by_percent(percent: number, forward?:boolean): [string, string] {
    const p_l_r = percent / 100;
  ...
  function drawChart() {
    const data: IChartData[] = process_back_end_data_in_range(); // ======>ok
    const scale = scaleAxis(); // ======== ok
    const scaleBar = scaleAxisY(); // ====== ok
    // down bars
    draw_down_bars({ data, scale, scaleBar }); // ======== ok
    // up bars
    if (chartType !== 'USER') {
      draw_up_bars({ data, scale, scaleBar }); // ========ok
    }
    // 创建横坐标轴
    if (appearanceConfig.axisHidden) {
      d3.select(`${randomId} .axis`).remove();
    ...
    if (appearanceConfig.hoverBoxHidden) {
      ...
      draw_background_bars({ data, scale, scaleBar }); // ======= ok
    }
    // remove select area
    if (chartType == 'USER' && removeParams) {
      draw_background_bars_for_select_area({ scale, scaleBar }); // ======== ok
    } else {
      d3.select('.remove_bars_background').remove();
    }
    ...
    if (appearanceConfig.controlHidden) {
      ...
      // init
      // drag left
      draw_drag_left({ scale }); // ======== ok
      // drag right
      draw_drag_right({ scale }); // ======= ok
    }
  }
  function process_back_end_data_in_range() {
    ...
    const data: IChartData[] = list.map((o: IChartData) => {
      const { point } = o;
      const price_r = reverse_price(get_price_by_point(+point));
      const point_max = +point + point_delta * bin_final;
      const price_l = reverse_price(get_price_by_point(point_max));
      return {
        ...o,
        liquidity: Big(o.liquidity || 0).toFixed(),
        order_liquidity: Big(o.order_liquidity || 0).toFixed(),
  ...
  function draw_background_bars_for_select_area({
    ...
    const max_bin_price = sortP[sortP.length - 1];
    const { fromLeft, fromRight, all, point } = removeParams;
    const remove_to_price = reverse_price(get_price_by_point(point));
  ...
  function draw_drag_left({ scale }: { scale: any }) {
    ...
    if (dragLeftPoint) {
      ...
      const newLeftPoint = get_nearby_bin_right_point(get_point_by_price(reverse_price(price_l_temp)));
    ...
    const dragLeft = d3.drag().on('drag', function (e) {
      ...
      if (rightX < e.x || e.x < dragBarWidth / 2) return;
      let p = reverse_price(scale.invert(e.x));
      const newLeftPoint = get_nearby_bin_right_point(get_point_by_price(p));
      setDragLeftPoint(newLeftPoint);
      setLeftPoint && setLeftPoint(newLeftPoint);
    });
    d3.select(`${randomId} .drag-left`).call(dragLeft);
  ...
  function draw_drag_right({ scale }: { scale: any }) {
    ...
    const dragRight = d3.drag().on('drag', (e) => {
      ...
      if (leftX > e.x - dragBarWidth / 2 || e.x > limitx) return;
      const p = scale.invert(e.x);
      const newRightPoint = get_nearby_bin_left_point(get_point_by_price(reverse_price(p)));
      setDragRightPoint(newRightPoint);
      setRightPoint && setRightPoint(newRightPoint);
    });
    d3.select(`${randomId} .drag-right`).call(dragRight);
  ...
  function get_price_and_liquidity_range() {
    ...
    list.forEach((o: IChartData) => {
      const { liquidity, price_l, price_r } = o;
      Y.push(+liquidity);
