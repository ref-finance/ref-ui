    ...
    <div className="mt-7">
      ...
      >
        <p className="flex justify-between items-center">
          ...
          <label className="text-xl text-white text-right">{totalPrice}</label>
        </p>
          ...
          <div className="flex flex-wrap mt-4">
            {rewardsList.map(
              (
                ...
                return (
                  <span className="flex items-center mr-8 mb-2" key={index}>
                    <img
                      src={icon}
                      className="w-7 h-7 rounded-full border border-gradientFromHover"
                    ></img>{' '}
                    <label className="text-sm text-white ml-2.5">
                      {userUnclaimedReward}
                    </label>
                  </span>
                );
